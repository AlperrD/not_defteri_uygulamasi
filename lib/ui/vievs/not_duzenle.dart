  ...
  void otoSave(bool didPop, Object? result) {
    if (didPop) {
      ...
      baslikController.text != widget.notlar.baslik || notController.text != widget.notlar.not
          ? {
              context
                  .read<NotDuzenleCubit>()
                  .guncelle(widget.notlar.notId, baslikController.text, notController.text, formattedDateTime),
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(notDuzenlendiText))),
            }
          : false; //ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(notDuzenlemeBasarisizText)));
    }
  }
          ...
          actions: [
            IconButton(
              ...
              onPressed: () {
                ...
                baslikController.text != widget.notlar.baslik || notController.text != widget.notlar.not
                    ? {
                        context.read<NotDuzenleCubit>().guncelle(
                            widget.notlar.notId, baslikController.text, notController.text, formattedDateTime),
                        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(notDuzenlendiText))),
                      }
